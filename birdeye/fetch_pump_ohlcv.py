...
def main():
    ...
    print(f"OHLCV interval: {args.interval}")
    ...
    for address in pbar:
        try:
            # Check if OHLCV history already exists
            ...
            # Get OHLCV history with explicit timestamps
            df = api.get_ohlcv_history(address, start_ts=start_ts, interval=args.interval)
            ...
            if df is not None and len(df) > 0:
                ...
                if args.verbose:
                    sources = ", ".join(address_to_source[address][:2])  # Limit sources shown
                    tqdm.write(f"OK   {address} | {len(df):>4} records | {sources} {security_status}")
            else:
                failed.append(address)
                if args.verbose:
                    tqdm.write(f"FAIL {address} | no data {security_status}")
